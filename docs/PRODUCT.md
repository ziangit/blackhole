...
- **Mass tracking** (background worker) — `mass` (0..1) in `chrome.storage.local`; fed by 5 s activity heartbeats while the tab is visible AND the user has produced input within ~90 s (scroll/pointer/key/mouse-move; scrolling accrues 1.5×) — AFK or passive autoplay does NOT feed the hole; `mass = clamp(effectiveSeconds / (limitMinutes*60), 0, 1)` with smoothstep near 1.0; starves via `chrome.alarms` decay (half-life `decayHalfLifeMinutes`, default 10) when no X tab is active; persists across restarts.
